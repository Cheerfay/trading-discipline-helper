...
> 最后更新：2026-06-22（对应分支 `homepage-content-seo-polish` 合并前状态）。
...
6. **首页双语**：首页文案已接入 Paraglide，默认英文路径 `/`，中文路径 `/zh`；顶部和底部都提供语言切换。不会根据浏览器语言自动跳转；英文首页仅在中文偏好浏览器且用户未明确选择过语言时，展示轻量中文切换提示。
7. **基础 SEO**：首页补充 canonical、hreflang、x-default、Open Graph 和 Twitter meta；`sitemap.xml` 先只收录当前准备好的首页、隐私政策、服务条款。
8. **结果页双语**：结果页固定 UI 已接入中英文；首页新生成请求会带当前 locale，冷静卡和仓位节奏卡的新生成内容会按当前语言输出。
9. **历史页双语**：历史页固定 UI 已接入中英文，包括标题、空状态、数量、场景标签、仓位前缀、删除弹窗和 SEO title。
...
- **双语仍有历史数据限制**：新生成的冷静卡/仓位卡会按当前语言输出；旧卡片和历史卡片中的模型正文不会自动翻译。法律页正文仍需后续单独处理，目前仍是模板通用文本。
...
| `homepage-content-seo-polish` | 当前工作分支：补充首页下方内容、FAQ、边界说明、基础 SEO、首页/结果页/历史页双语基础 |
